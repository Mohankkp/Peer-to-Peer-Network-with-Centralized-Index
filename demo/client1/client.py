...
class Client(object):
    def __init__(self, ServerHost='127.0.0.1', Version='P2P-CI/1.0', dir='RFC_LIST'):# Constructor
        ...
        self.shareable = True#Interprocess shareables

    def start(self): #Start Connection to the Server 
        print('Connecting to the server %s on Port : %s' %(self.ServerHost, self.ServerPort))
        self.server = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #Create an IPV4-TCP socket object to connect to the server
        try:#Initiate Connection to the server
            ...
            return   
        uploader_process = threading.Thread(target=self.Pre_upload) #Create a new process
    ...
    def Client_Requests(self): #Request a Service
        ...
        while True:
            try:
                ...
                req = input('\n 1: Add\n 2: Look Up\n 3: List All\n 4: GetRFC\n 5: Terminate\n')
                Request_Methods.setdefault(req, self.Invalid_Command)()#Service the Request and If requested for Unavailable service throw an error
    ...
    def ADD(self, num=None, title=None):#Subroutine to ADD RFC to the server database
        if not num:
            num = input('Enter the RFC number to be added:')#Get the RFC nummber to be added
            if not num.isdigit():#If Valid number is not entered throw an error
    ...
    def DOWNLOAD(self):#Subroutine to initiate download of an RFC from a peer
        ...
        if lines[0].split()[1] == '200':
            ...
            # send get request
            self.GetRFC(num, title, peer_host, peer_port)
    ...
    def GetRFC(self, num, title, peer_host, peer_port):#Subroutine to Download RFC from a Peer
        try:
            # make connnection
    ...
    def Pre_upload(self):#Subroutine to initiate Peer to Peer Communication
        ...
        self.uploader.bind(('', 0)) #Allocation of an avaialble free port or Bind to a free TCP port
        self.Upload_Port = self.uploader.getsockname()[1]#Get the Port number
        self.uploader.listen(5)#Start Listening on Upload Port
        while self.shareable:
            requester, address = self.uploader.accept()
            handler = threading.Thread(target=self.Upload_Process, args=(requester, address))#Creating a Thread to Open New Connection
            handler.start()#Starting a thread
        self.uploader.close()#Once the function is completed close the thread

    def Upload_Process(self, soc, address):#Peer to Peer Data transfer
        header = soc.recv(1024).decode().splitlines()
        try:
            version = header[0].split()[-1] #Store the version
            num = header[0].split()[-2]#Store the Port Number
